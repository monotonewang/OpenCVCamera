...
public class MainActivity extends Activity implements CvCameraViewListener2, OnClickListener {
    ...
    private ImageView matchDrawArea;
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        ToggleButton thHomo = (ToggleButton) findViewById(R.id.tbHomo);
        thHomo.setOnCheckedChangeListener(new OnCheckedChangeListener() {
            @Override
        ...
        ((ToggleButton) findViewById(R.id.tbImage)).setOnCheckedChangeListener(new OnCheckedChangeListener() {
            @Override
    ...
    public Mat onCameraFrame(CvCameraViewFrame inputFrame) {
        last = inputFrame.rgba();//从视频返回一个rgba的mat矩阵
        return last;
    }
    ...
    public void onClick(View view) {
        switch (view.getId()){
            case R.id.btnClear:
                scenes.clear();
                btnAdd.setText("Add (" + scenes.size() + ")");
                break;
            case R.id.btnAdd:
                ...
                break;
            case R.id.btnRefer:
                Mat im = last.clone();//创建一张图像的图片矩阵
                // Imgproc.cvtColor(im, im, Imgproc.COLOR_BGR2RGB);
                Bitmap bmp = Bitmap.createBitmap(im.cols(), im.rows(),
                        Bitmap.Config.ARGB_8888);//得到一张bitmap
                Utils.matToBitmap(im, bmp);//把android的bitmap转化为opencv里面的矩阵
                matchDrawArea.setImageBitmap(bmp);
                refScene = new Scene(last);//创建一个场景类
                break;
            case R.id.btnFind:
                if (scenes.size() == 0) {
                    AlertDialog alertDialog = new AlertDialog.Builder(this).create();//创建一个对话框
                    ...
                    new BetterComparePics(this).execute();//开始异步执行
                }
                break;
        }
    }
